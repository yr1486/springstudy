...
<script>
	...
	function fnGetEmployees(){
		...
		$.ajax({
			...
			data:'page=' + page,  // page=1, page=2, page=3... 으로 동작 // 보내주는 파라미터 : 파라미터페이지보내주기로함
			// 응답
			dataType: 'json',
			success: function(resData){ // resData = {"employes": [{},{}, {}, ...], "totalPage": 12}
	...
	// 스크롤 이벤트
	$(window).on('scroll', function(){			// 스크롤 된 길이
	
		// 500밀리초(0.5초) 후에 function을 수행한다.
		
		if(timerId) {
			clearTimeout(timerId); // setTimeout()이 동작했다면(목록을 가져왔다면) setTimeout()의 재동작을 취소한다.(동일 목적을 가져오는 것을 방지한다.)
		}
		// timerId = setTimeout(function(){}, 500);
		timerId = setTimeout(function(){
			
			let scrollTop = $(this).scrollTop();	// 화면 높이(브라우저의 크기)
			let windowHeight = $(this).height();
			let documentHeight = $(document).height;	// 문서 높이
			if((scrollTop + windowHeight + 50) >= documentHeight) { // 스크롤이 바닥에 닿기 전 50픽셀정도 위치
