...
public class UploadServiceImpl implements UploadService {
	...
	@Transactional(readOnly = true) // 리드온리 붙여주면 트랜잭셔널 성능이 향상된다. // INSERT문을 2개 이상 수행하기 떄문에 트랜젝션 처리가 필요하다.
	@Override
	public int addUpload(MultipartHttpServletRequest multipartRequest) {
		...
		int addResult = uploadMapper.addUpload(uploadDTO); // <selectkey>에 의해서, 업로드디티오객체의 업로드넘버 필드에
																// UPLOAD_SEQ.NEXTVAL
